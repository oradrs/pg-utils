...
totals as (
	select sum(total_exec_time) AS total_exec_time, greatest(sum(blk_read_time+blk_write_time), 1) as io_time,
...
_pg_stat_statements as (
    ...
    query_normalized as query, sum(total_exec_time) as total_exec_time,
...
statements as (
	...
	where ((total_exec_time-blk_read_time-blk_write_time)/(select cpu_time from totals)>=0.01 or (blk_read_time-blk_write_time)/(select io_time from totals)>=0.01 or calls/(select ncalls from totals)>=0.02)
union all
	select
	...
	'other' as query
	from _pg_stat_statements
	where not ((total_exec_time-blk_read_time-blk_write_time)/(select cpu_time from totals)>=0.01 or (blk_read_time-blk_write_time)/(select io_time from totals)>=0.01 or calls/(select ncalls from totals)>=0.02)
),
...
select E'total time:\t' || total_exec_time || ' (IO: ' || io_time_percent || E'%)\n' ||
E'total queries:\t' || total_queries || E'\n' ||
E'unique queries:\t' || unique_queries || E'\n\n'
from totals_readable
union all
(select E'=============================================================================================================\n' ||
'pos:' || pos || E'\t total time: ' || total_exec_time || ' (' || time_percent || ', CPU: ' || cpu_time_percent || ', IO: ' || io_time_percent || E')\t calls: ' || calls ||
